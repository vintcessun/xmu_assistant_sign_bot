...
| `/setpwd` | `/setpwd` | 设置或刷新访问口令。发送后给出一次性链接（15 分钟内有效、只能用一次），在网页上手输或点「随机生成」保存即可。口令**长期有效**、绑定 QQ，所有需要口令的页面都用它解锁；想换口令就再发一次 `/setpwd`，旧口令立即失效。服务端只存摘要（随机盐 + 10 万轮 SHA-256），明文不落盘不进日志。 |
| `/jdpm` | `/jdpm` | 创建绩点/排名查询网页并发送链接（30 分钟内有效），用 `/setpwd` 设的口令解锁。选一个成绩范围点查询，会一次性返回绩点、加权/算术平均分，并自动申请计算（先查后申，已有有效结果不重复提交）、下载绩点证明 PDF、从正文里提取出**专业绩点排名**与专业总人数，同时展示完整证明正文并提供 PDF 下载。排名是关键：教务的 JSON 接口一律返回 `*`，只有证明 PDF 里才写明。 |
...
- `src/web`：基于 `axum` 的 Web 暴露子系统，`build.rs` 按目录自动组装路由。主要模块：`file`（大文件流式下载）、`md`（Markdown 预览与 PDF 导出）、`login` / `vpn`（登录与 SecureLink 登录页）、`timetable`、`rollcall`、`guard`（访问口令：按 QQ 绑定的长期口令，盐 + 多轮 SHA-256 摘要落盘，一次性链接设置/刷新，令牌轮换保证同一时刻只有一人在用，失败锁定）、`jdfpm`（绩点与排名查询页，受 `guard` 保护）。
