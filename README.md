...
| 命令未命中路由 (`routing_miss_all`) | 未匹配消息的过滤与退出时长 | `2.64 µs` | 性能提升 (-52.25%) |
...
| 场景 | 核心指标 | 最新耗时 | 性能变化 |
|---|---|---|---|
| 冷存储读取命中 (`cold_get_hit`) | 从 Redb 冷存储中读取单个 Key 的耗时 | `828.32 ns` | **性能显著提升 (-92.73%)** |
| 高并发热存储吞吐 (`hottable_concurrent_read_write_x100_90_10`) | 100路并发读写 DashMap 耗时 | `51.941 µs` | 轻微退化 (+10%) |
| 冷存储写入 (`cold_insert`) | 写入 Redb 冷存储耗时 | `2.155 ms` | 性能提升 (-9.4%) |
...
- **零克隆优化**:
    ...
    - **消息ID引用优化**: 优化了消息归档和回复上下文重建流程中对消息 ID 的处理，通过在外部使用引用，避免了在 `SegmentReceive::Reply` 和 `SegmentReceive::Forward` 中进行冗余的 `String::clone()`，大幅提升了相关处理的 I/O 效率。
    - **字符串与所有权**:
...
- **工具调用增强**: 新增联网获取网页和执行 Python 代码的工具调用功能，极大地扩展了 LLM 的能力边界。
- **`#[tool]` 宏引入**: 添加 `tool` 宏用于自动生成 LLM 工具所需的样板代码，简化新工具的开发流程。
- **LLM 类型安全封装**: 进一步强化 LLM 结构化解析的健壮性：
    - 修复了 `LlmPrompt` 宏无法正确解析 `prompt` 属性的问题。
    - 引入 `LlmEnum` 解决 `enum` 类型的解析问题，并新增 `LlmF64` 支持浮点数。
    - 修复 `String` 类型内部提示词，强制要求 LLM 使用 `CDATA` 格式返回，确保 XML 解析的稳定性。
    - 修复 `LlmHashMap` 内部采用 `BTree` 实现，确保键值对的迭代和序列化顺序稳定。
    - 修复 `LlmType` 泛型无稳定 `static` 的问题，并为其添加所有必要属性，提高结构化解析的准确性。
...
- **Cold 层 (Redb)**: 采用 Copy-on-Write (CoW) 机制的嵌入式数据库，通过异步 Task 批量提交事务，完全剥离主循环 I/O。**新增 `get_all()` 遍历方法**，支持在服务启动时加载全部数据用于重建内存索引 (#6)。修复了 `ColdDb` 的表初始化逻辑，确保在创建 `ColdTable` 实例时存储表一定存在。
- **Hot 层和 Cold 层定长 bincode**: 存储层使用定长的 `bincode` 格式，牺牲少量空间换取更一致、更优秀的序列化/反序列化性能，有效提升 `cold_get_hit` 性能。
- **网络性能优化**: 延长网络连接的 TCP Keep-Alive 时间（3600s）并增加连接池大小（每主机 100），显著提升了请求性能和连接复用率。
...
- **向量搜索引擎与 RAG 引擎**:
    ...
    - **日志增强**: 增加了向量索引创建和搜索过程的日志详细程度（Trace 级别），便于调试和性能分析。
...
- **Deep Reply 策略调整**: 修复了 Deep 回复的提示词，要求 LLM 只有在明确提问时才回复，有效减少了不必要的回复频率，优化用户体验。
- **日志级别调整**: 将 LLM 的 `token` 使用和查询等信息日志从 `info` 级别降为 `debug` 级别，以减少正常运行时的日志冗余。
- **考试 API 修复**: 修复了 LNT `exams` API 返回的时间字段类型，现在使用 `chrono::DateTime<Utc>` 替代 `String`，方便进行时间操作和格式化。
- **指令时间修复**: `/test` 指令返回的小测时间现在已正确转换为北京时间。
- **Workflow 修复**: 尝试修复了 CI/CD `workflow` 编译错误。
- **编译优化**: 升级了编译优化配置，生成效率更高的二进制文件。
- **智能 JSON**: 提供了更加智能的 JSON 解析函数，在 Release 模式下等效于原本的快速 JSON 函数，同时提升了开发和报错体验。

### 10. Expose 模块：带上下文的文件/内容暴露
