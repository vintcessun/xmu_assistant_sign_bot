...
fn main() {
    ...
    // --- 逻辑 G: 生成 Web 静态资源 ---
    generate_web_data(&out_dir);

    // --- 逻辑 H: 生成 web 路由模块 ---
    generate_web_router_mod();

    // --- 逻辑 I: 生成赞赏码图片数据 ---
    generate_reward_data(&out_dir);
}
...
/// 赞赏码是资源文件，和二维码模型一样编译期嵌进二进制，运行期再释放到数据目录。
fn generate_reward_data(out_dir: &str) {
    let dest_path = Path::new(out_dir).join("reward_data.rs");
    const REWARD_PNG: &str = "images/mm_reward_qrcode_1782913890167.png";

    println!("cargo:rerun-if-changed={REWARD_PNG}");

    // 不用 canonicalize：Windows 上它会给出带 UNC 前缀的路径，塞进 include_bytes! 还要额外清理。
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let path = Path::new(&manifest_dir).join(REWARD_PNG);
    assert!(path.exists(), "缺少赞赏码图片 {REWARD_PNG}");

    let code = format!(
        r###"// NOTE: This file is automatically generated by build.rs. Do not edit manually.

pub const REWARD_QRCODE_PNG: &[u8] = include_bytes!(r#"{}"#);
"###,
        path.display()
    );
...
fn generate_web_data(out_dir: &str) {
    let dest_path = Path::new(out_dir).join("web_data.rs");
    let web_dir = Path::new("app_data/web");

    println!("cargo:rerun-if-changed=app_data/web");
    ...
    if let Ok(entries) = fs::read_dir(web_dir) {
        ...
        for entry in entries {
            ...
            if path.extension().is_some_and(|e| e == "html") {
                ...
                let var_name = format!("{}_HTML", file_stem.to_uppercase());
...
fn generate_web_router_mod() {
    let web_root = Path::new("src/web");
    let dest_path = web_root.join("mod.rs");

    println!("cargo:rerun-if-changed=src/web");

    let mut module_names = Vec::new();

    let mut entries: Vec<_> = fs::read_dir(web_root)
        .expect("Failed to read src/web directory")
        .flatten()
        .collect();
    entries.sort_by_key(|e| e.file_name());
    ...
    let mut router_nests = String::new();

    for module in &module_names {
        mod_declarations.push_str(&format!("pub mod {};\n", module));
        router_imports.push_str(&format!("use {}::{}_router;\n", module, module));
        router_nests.push_str(&format!(
            "    let router = router.nest(\"/{0}\", {0}_router());\n",
            module
        ));
    }
...
{router_nests}    main_router(router)
}}
...
"#
    ...
    fs::write(dest_path, generated).expect("Failed to write src/web/mod.rs");
}
