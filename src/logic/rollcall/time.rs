...
static START_DATE: LazyLock<NaiveDate> =
    LazyLock::new(|| NaiveDate::from_ymd_opt(2026, 3, 2).unwrap());
...
fn get_week_number(start_date: NaiveDate, target_date: NaiveDate) -> (i32, Weekday) {
    ...
    (
        (days / 7) as i32 + 1,
        Weekday::from_i32((days % 7) as i32 + 1).unwrap(),
    )
}
...
pub fn get_today_courses(qq: i64) -> Option<TimeBitMap> {
    ...
    for course in &timetable_data.times {
        if course.day == day_number && course.week_mask.get_bit((week_number - 1) as u8) {
